...
FROM CHALLENGE) C
WHERE C.RNUM >= 1 AND C.RNUM <=7
ORDER BY C.RNUM, C.CHALLENGESDATE DESC


--유저 챌린지 참가갯수 많은 순(챌린지 인증 많이)
SELECT ROW_NUMBER()OVER(PARTITION BY CHALLENGESEQ ORDER BY CHALLENGESEQ DESC) AS RNUM, COUNT(*) AS CERTIFYCOUNT
FROM CERTIFY
GROUP BY CHALLENGESEQ
--유저 달성률이 높은 순(인증많이)
...
SELECT COUNT(*) FROM CHALLENGE
		WHERE  CHALLENGEDEL = 0
				...
				AND CHALLENGESTART > TO_DATE('2021-06-20 18:00', 'YYYY-MM-DD HH24:MI:SS')
