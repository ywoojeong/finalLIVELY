...
--자료형 사이즈 변경
ALTER TABLE CHALLENGE MODIFY CHALLENGEPHOTO VARCHAR2(500)
ALTER TABLE CHALLENGE MODIFY CHALLENGESAVEPHOTO VARCHAR2(500)
ALTER TABLE CHALLENGE MODIFY CERTIFYPHOTO VARCHAR2(500)
ALTER TABLE CHALLENGE MODIFY CERTIFYSAVEPHOTO VARCHAR2(500)
--1	건강
--2	역량
--3	정서
--4	자산
...
--챌린지 유저 많은 순
SELECT C.*
FROM (SELECT ROW_NUMBER()OVER(ORDER BY CHALLENGEMEMBER DESC, CHALLENGESTART DESC) AS RNUM, CHALLENGETITLE, CHALLENGEMEMBER 
FROM CHALLENGE) C
...
--포인트 많은 순
SELECT C.*
FROM (SELECT ROW_NUMBER()OVER(ORDER BY POINT DESC) AS RNUM, EMAIL, NICKNAME, MEMBERPHOTONAME, POINT
FROM MEMBER) C
WHERE C.RNUM BETWEEN 1 AND 7

--팔로잉 많이 받은 순
SELECT F.*
FROM (SELECT ROW_NUMBER() OVER(ORDER BY FOLLOWINGEMAIL DESC) AS RNUM, COUNT(*) AS FOLLOWING, FOLLOWINGEMAIL
FROM FOLLOW
GROUP BY FOLLOWINGEMAIL) F
...
-- 전체 유저 가입 월별
SELECT TO_CHAR(B.DT, 'YYYY-MM') AS MEMBERSDATE, NVL(SUM(a.CNT), 0) AS MEMBERCOUNT
  FROM (SELECT TO_CHAR(MEMBERSDATE, 'YYYY-MM-DD') AS MEMBERSDATE, COUNT(*) AS CNT
      ...
      (SELECT TO_DATE('2021-01-01','YYYY-MM-DD') + LEVEL - 1 AS DT
       FROM dual 
       CONNECT BY LEVEL <= (TO_DATE('2021-12-31','YYYY-MM-DD') - TO_DATE('2021-01-01','YYYY-MM-DD') + 1)
       ) B
...
-- 전체 챌린지 생성 월별
SELECT TO_CHAR(B.DT, 'YYYY-MM') AS CHALLENGESDATE, NVL(SUM(A.CNT), 0) AS CHALLENGECOUNT
  ...
  SELECT TO_CHAR(b.dt, 'YYYY-MM') AS MEMBERSDATE, NVL(SUM(a.cnt), 0) AS cnt
  FROM (SELECT TO_CHAR(MEMBERSDATE, 'YYYY-MM-DD') AS MEMBERSDATE, COUNT(*) AS cnt
        FROM MEMBER
        WHERE MEMBERSDATE BETWEEN TO_DATE('2021-01-01', 'YYYY-MM-DD') AND TO_DATE('2021-12-31', 'YYYY-MM-DD') 
        GROUP BY TO_CHAR(MEMBERSDATE, 'YYYY-MM')
        ) a,
      (SELECT TO_DATE('2021-01-01','YYYY-MM-DD') + LEVEL - 1 AS dt
       FROM dual 
       CONNECT BY LEVEL <= (TO_DATE('2021-12-31','YYYY-MM-DD') - TO_DATE('2021-01-01','YYYY-MM-DD') + 1)
